...
추출 기준:
- 위 제스처 단어(썰기/젓기/볶기/굽기 등)가 자막 텍스트에 **그대로 등장**할 때만 추출
- 자막에 제스처 단어가 없으면 절대 추출하지 말 것 — 유사 행동으로 유추하거나 대체하지 말 것
- 주방 도구가 확정된 경우, 해당 도구를 사용하는 행동만 추출
- 행동 설명은 "감자 썰기", "양파 볶기" 처럼 [재료 + 제스처] 형식으로 작성
- gesture 필드에는 반드시 위 [인식 가능한 제스처] 목록 중 하나만 입력할 것
- 다른 텍스트는 절대 포함하지 말고 JSON 배열만 반환

반드시 제외할 항목:
- "넣기", "넣으세요", "추가하세요", "올리세요" 등 제스처 목록에 없는 행동
